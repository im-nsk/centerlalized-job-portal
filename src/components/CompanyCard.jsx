...
export default function CompanyCard({
  ...
  return (
    <div className="jcc-card jcc-card-hover" style={{ padding: 16, display:'flex', flexDirection:'column' }}>
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom: 12 }}>
        ...
        >
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: 14.5, fontWeight: 550, lineHeight: 1.2, overflow:'hidden', textOverflow:'ellipsis', display:'-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{company.name}</div>
      ...
      <div className="jcc-action-grid" style={{ marginTop:'auto' }}>
        <button
          type="button"
        ...
        >
          {careerLoading ? <LoadingSpinner size={14}/> : smartSearchMode ? <Sparkles size={12}/> : <ExternalLink size={12}/>}
          View Roles
        </button>
        <button type="button" className="jcc-btn" onClick={onOpen} style={{ justifyContent:'center' }}>
          <Edit3 size={12}/> Track
        </button>
        <button type="button" className="jcc-btn" onClick={onEmployees} style={{ justifyContent:'center' }} title="LinkedIn search for engineers at this company">
          <Users size={12}/> Employees
        </button>
        <button type="button" className="jcc-btn" onClick={onRecruiters} style={{ justifyContent:'center' }} title="LinkedIn search for recruiters">
